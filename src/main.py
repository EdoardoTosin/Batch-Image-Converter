...
group_opt = parser.add_argument_group('other options', 'Customize script behavior (alert and wait)')

parser.add_argument("-v", "--version", action='version', version=__version__)
parser.add_argument("--update", action='store_true', help="check latest version available")
...
group_img.add_argument("-p", "--path", type=dir_path, default=current_path, help=f'path where images are located (default: "{current_path}")')
group_img.add_argument("-d", "--dpi", type=int, choices=range(1, 1001), metavar="[1-1000]", default=72, help="pixel density in dpi (default: 72)")
group_img.add_argument("-s", "--size", type=int, choices=range(1, 10001), metavar="[1-10000]", default=1000, help="max resolution of image (long side) in pixels (default: 1000)")
group_img.add_argument("-f", "--filter", type=int, choices=range(0, 6), metavar="[0-5]", default=0, help="type of filter used for downscaling (default: 0 = Nearest)")
group_img.add_argument("--colorspace", "--cs", type=bool, default=False, action=argparse.BooleanOptionalAction, help="convert all images to RGB color space")
group_img.add_argument("-q", "--quality", type=int, choices=range(1, 101), metavar="[1-100]", default=80, help="quality of output images (default: 80)")
group_img.add_argument("-m", "--max-image-mpixels", type=int, choices=range(0, 10001), metavar="[0-10000]", default=0, help="maximum image resolution allowed in Megapixels (default: 0 [None])")
group_img.add_argument("--optimize", type=bool, default=True, action=argparse.BooleanOptionalAction, help="attempt to compress the palette by eliminating unused colors")
group_opt.add_argument("--alert", type=bool, default=True, action=argparse.BooleanOptionalAction, help="play alert sound when finished the conversion")
group_opt.add_argument("--wait", type=bool, default=True, action=argparse.BooleanOptionalAction, help="wait for user keypress (Enter) when finished the conversion")
...
def print_init(args):
    ...
    if args.colorspace:
        if not os.path.exists(cmyk_profile_path) or not os.path.exists(srgb_profile_path):
            print(f" -> {Fore.YELLOW}WARNING: ICC profile files not found. Skipping CMYK to sRGB conversion.")
        else:
...
def convert_to_srgb(img):
    ...
    img = ImageCms.profileToProfile(img, cmyk_profile_path, srgb_profile_path, outputMode="RGB")
    return img
...
def main(args):
    ...
    if args.colorspace and not (os.path.exists(cmyk_profile_path) and os.path.exists(srgb_profile_path)):
