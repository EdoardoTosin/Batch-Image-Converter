...
def start_proccess(
    ...
    run = Proccess(
        ...
        max_image_mpixles=max_image_mpixles_data,
...
inputs_title = CTkLabel(
    root, text="Data fields : ", font=("Arial", 17), text_color="green"
).pack()
...
path = CTkEntry(
    inputs,
    placeholder_text=f"path where images are located (default: {current_path})",
    width=1100,
    height=35,
)
...
size = CTkEntry(
    inputs, placeholder_text="Max resolution (default: 1000)", width=420, height=30
)
size.grid(row=1, column=1, pady=10, padx=10)

filter = CTkEntry(
    inputs,
    placeholder_text="type of filter used for downscaling (default: 0 = Nearest)",
    width=1100,
    height=30,
)
...
quality = CTkEntry(
    inputs,
    placeholder_text="quality of output images (default: 80)",
    width=420,
    height=30,
)
quality.grid(row=3, column=0, pady=15)

max_image_mpixles = CTkEntry(
    inputs,
    placeholder_text="maximum image resolution allowed in Megapixels (default: 0 [None])",
    width=420,
    height=30,
)
...
colorspace = CTkCheckBox(
    inputs, text="convert all images to RGB color space", onvalue=True, offvalue=False
)
colorspace.grid(row=4, column=0, pady=5)

optimize = CTkCheckBox(
    inputs,
    text="attempt to compress the palette by eliminating unused colors",
    onvalue=True,
    offvalue=False,
)
...
alert = CTkCheckBox(
    inputs,
    text="play alert sound when finished the conversion",
    onvalue=True,
    offvalue=False,
)
...
wait = CTkCheckBox(
    inputs,
    text="wait for user keypress (Enter) when finished the conversion",
    onvalue=True,
    offvalue=False,
)
...
start_btn = CTkButton(
    ...
    height=33,
    font=("Arial", 15),
    text="Start conveting",
    corner_radius=10,
    hover=False,
    command=lambda: start_proccess(
        path=path,
        dpi=dpi,
        size=size,
        filter=filter,
        quality=quality,
        max_image_mpixles=quality,
